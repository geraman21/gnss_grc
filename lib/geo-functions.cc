...
MatrixXd e_r_corr(double traveltime, Vector3d X_sat) {
  ...
  R3 << cos(omegatau), sin(omegatau), 0, -sin(omegatau), cos(omegatau), 0, 0, 0, 1;
  ...
  for (int iter = 0; iter < nmbOfIterations; iter++) {
    for (int i = 0; i < nmbOfSatellites; i++) {
      ...
      if (iter == 0) {
        ...
        double travelTime = sqrt(rho2) / (double)c;
      ...
      A.row(i) << (-(Rot_X(0) - pos(0))) / obs.at(i), (-(Rot_X(1) - pos(1))) / obs.at(i),
          (-(Rot_X(2) - pos(2))) / obs.at(i), 1;
    }
    MatrixXd x;

...
Ephemeris::Ephemeris(std::vector<int> &navBits, int channel) {
  ...
  for (int i = 1; i <= 5; i++) {
    std::vector<int> subframe(navBits.begin() + 300 * (i - 1), navBits.begin() + 300 * i);
    ...
    for (int j = 1; j <= 10; j++) {
      if (D30Star == 1) {
        for (int a = 0; a < 24; a++) {
          subframe.at(30 * (j - 1) + a) == 0 ? subframe.at(30 * (j - 1) + a) = 1
    ...
    case 2:
      ...
      M_0 = twosComp2dec(vecSelector(subframe, 107, 114, 121, 144)) * pow(2, -31) * gpsPi;
    ...
    case 3:
      ...
      omega_0 = twosComp2dec(vecSelector(subframe, 77, 84, 91, 114)) * pow(2, -31) * gpsPi;
      C_is = twosComp2dec(vecSelector(subframe, 121, 136)) * pow(2, -29);
      i_0 = twosComp2dec(vecSelector(subframe, 137, 144, 151, 174)) * pow(2, -31) * gpsPi;
      C_rc = twosComp2dec(vecSelector(subframe, 181, 196)) * pow(2, -5);
      omega = twosComp2dec(vecSelector(subframe, 197, 204, 211, 234)) * pow(2, -31) * gpsPi;
      omegaDot = twosComp2dec(vecSelector(subframe, 241, 264)) * pow(2, -43) * gpsPi;
